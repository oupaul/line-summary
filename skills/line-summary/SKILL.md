...
**驗證方式，不要只看 shell 指令本身的 exit code**：如果這個 scp 是用
`run_in_background` 執行、或跟其他指令用 `;`／`&&` 接在同一個 Bash 呼叫裡
（例如 `scp ...; echo 結果`），最後回傳的 exit code 是**最後一個指令**的，
不是 scp 真正的成功與否——之前就因為這樣把逾時失敗誤判成功過一次。同步後
一定要另外對遠端跑 `ssh ... "ls -la <遠端子資料夾>/"`，實際看到檔案跟時間
戳對得上，才能在回覆裡說「同步成功」。

同步結果（成功或失敗，含失敗原因）在回覆裡如實講一句，不要靜默失敗或假裝
成功。`local-context.md`
